...
public class NodeFolderManager: ObservableObject {
    ...
    public func renameNodeFolder(from oldNode: Node, to newNode: Node) throws {
        ...
        do {
            ...
            if hasMarkdownFile {
                ...
                // 更新图片链接：从绝对路径转换为相对路径
                let updatedContent = updateImageLinksToRelativePaths(content: markdownContent, oldFolderName: oldFolderName, oldNode: oldNode, newNode: newNode)
    ...
    private func updateImageLinksToRelativePaths(content: String, oldFolderName: String, oldNode: Node, newNode: Node) -> String {
                ...
                if match.numberOfRanges >= 3 {
                    ...
                    if let fullMatch = Range(fullMatchRange, in: content),
                       ...
                       let pathRng = Range(pathRange, in: content) {
                        ...
                        // 检查是否需要更新路径（任何复杂路径都简化为文件名）
                        if originalPath != description && !originalPath.isEmpty {
                            // 提取文件名（路径的最后一部分）
    ...
    public func saveImageToNodeFolder(_ node: Node, fileName: String, data: Data) throws -> String {
        ...
        // 🎯 最简单的方案：图片和MD在同一文件夹，直接用文件名
        print("🖼️ 生成相对路径（同文件夹）: \(fileName)")
        return fileName
    }
