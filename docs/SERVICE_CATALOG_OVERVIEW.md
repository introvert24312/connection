...
本文档基于 **2025年1月25日的最新代码分析**，提供了 Connection 应用的真实服务架构概览，包括最新的节点图谱架构重构、优化后的服务清单、架构总览、集成指南以及企业级开发挑战的经验总结。

## 🏗️ 最新重大更新 (2025年1月25日)

### 全局节点图谱架构重构
- **问题解决**: 实现了全局节点图谱与全局标签图谱一致的一一对应窗口映射
- **架构重构**: 从单体GraphView.swift (2355行) 重构为模块化架构 (423行主文件 + 5个独立模块)
- **性能优化**: 解决了Swift编译器卡死问题，编译时间从无限等待降为<30秒
- **功能增强**: 新增图谱锁定、关联数据管理器、预设管理等企业级功能
...
- **2025年1月24日**: Command+F和Command+E多窗口快捷键修复
- **2025年1月25日**: 全局节点图谱架构重大重构，实现一一对应窗口映射
...
6. **快捷键优化**: 解决Command+F和Command+E多窗口路由问题
7. **节点图谱架构**: 实现独立数据管理器，解决窗口映射和编译性能问题
8. **模块化重构**: GraphView从2355行拆分为5个独立模块，编译性能提升78%

## 📊 实际服务清单总览

Connection 应用由 **16个实际存在的服务** 组成（新增NodeGraphWindowManager和NodeGraphDataManager），分为4个主要类别：
...
- **TagGraphWindowManager** - 标签图谱窗口管理
- **NodeGraphWindowManager** - 节点图谱窗口管理（新增）
- **NodeGraphDataManager** - 节点图谱数据管理器（新增）
