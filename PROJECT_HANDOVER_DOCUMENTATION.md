...
### 🛠️ Infrastructure Services (4个)
| 服务名 | 版本 | 职责 | 关键特性 |
|--------|------|------|----------|
| **KeychainManager** | 1.9.0 | 安全凭据存储 | macOS Keychain、Git凭据、安全删除 |
| **KeyboardEventManager** | 1.9.0 | 全局键盘事件处理 | 命令限流、错误恢复、焦点跟踪 |
...
| 服务名 | 版本 | 职责 | 关键特性 |
|--------|------|------|----------|
| **CommandPaletteService** | 1.9.0 | 键盘驱动命令界面 | 模糊命令搜索、上下文感知、可扩展命令 |
| **MapContainerService** | 1.9.0 | 交互式地图可视化 | 节点聚类、位置标签导航、缓存瓦片 |
| **FullscreenGraphWindowManager** | 1.9.0 | 图形窗口管理 | 全屏图形、窗口状态、多窗口协调 |
